...
%             'GRIN009_H13_M59_S14', 'GRIN011_H10_M19_S59_01', ...
...
sessions = {'GRIN011_H10_M19_S59_01'};

inputPeakThreshold = 2.5 ; 
inputMicronsPerPixel = 0.9 ; % micronsPerPixel 2.5 = microns (inscopix), 1 (v3), 1.85 (v4)
inputMaxDist = 500 ; 
inputBinSize = 225 ; 
inputBStart = 50 ;
...
analysisOutputs = cell(3, length(sessions));
for i=1:sizeSessions(1,2)
    session=sessions{1,i} ;
    [outputFileArray, frameFileNames]  = analyzeJaccardsForSessionFn(session, dirInput, regExp, CNMFE_path, inputPeakThreshold, inputMicronsPerPixel, ...
            inputMaxDist, inputBinSize, inputBStart, inputNumBins) ; 
    analysisOutputs{1,i}=session; 
    analysisOutputs{2,i}=outputFileArray;
    analysisOutputs{3,i}=frameFileNames;
